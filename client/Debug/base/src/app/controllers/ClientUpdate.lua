...
function ClientUpdate:ctor(newfileurl,savepath,curfile,downurl)
	self._newMD5Url = newfileurl
	self._savepath = savepath
	self._curMD5File = curfile
	self._downUrl = downurl

	cclog("function ClientUpdate:ctor(newfileurl,savepath,curfile,downurl) ==>")
	cclog("newfileurl:  "..newfileurl)
	cclog("savepath:  "..savepath)
	cclog("curfile:  "..curfile)
	cclog("downurl:  "..downurl)
...
function ClientUpdate:upDateClient(listener)
	cclog("function ClientUpdate:upDateClient(listener)==>")
	--监听
	...
	do
		...
		if not createDirectory(self._savepath) then
			msg = "创建保存"..self._savepath.."文件夹失败！"
			break
		end

		--获取当前文件列表
		local szCurList = cc.FileUtils:getInstance():getStringFromFile(self._curMD5File)
		local oldlist = {}
		...
		appdf.onHttpJsionTable(self._newMD5Url,"GET","",function(jsondata,response)
				--记录新的list
				...
				if jsondata then
					...
					for k,v in pairs(oldlist) do
						...
						if  oldpath then
							...
							--删除文件
							if bdel == true then
								cclog("removefile:"..self._savepath..oldpath..oldname)
								fileUtil:removeFile(self._savepath..oldpath..oldname)
							end
						end
					end
					--下载判断
					cclog("ClientUpdate:upDateClient==newlist: " .. cjson.encode(newlist) .. "=oldList: " .. cjson.encode(oldlist))
					for k ,v in pairs(newlist) do
						...
						if newpath then
							...
							for oldk,oldv in pairs(oldlist) do
								...
								if oldpath == newpath and newname == oldname then 
									if newmd5 == oldmd5 then
										needupdate = false -- test:全部先设置为True,稍后再改回来: needupdate = false
									end
									break
								end
							end
							...
							if needupdate == true then
								cclog(newname.."-needupdate")
								table.insert(this._downList , {newpath,newname})
							end
						end
						
					end
					cclog("ClientUpdate:upDateClient===update_count:".. #this._downList)
					--开始下载
					if #this._downList >0 then
						...
						cc.FileUtils:getInstance():writeStringToFile(self._response,self._curMD5File)
...
function ClientUpdate:UpdateFile()
	...
	cclog("from ==> " .. url .. " download " .. fileinfo[2])
	--调用C++下载
	downFileAsync(url,fileinfo[2],dstpath,function(main,sub)
			...
			elseif main == appdf.DOWN_COMPELETED then --下载完毕
				cclog("ClientUpdate:UpdateFile===finish=" .. fileinfo[2])
				self._retryCount = 3;
				...
				cclog("ClientUpdate:UpdateFile===fail=" .. fileinfo[2])
