...
function GameFrameEngine:ctor(view,callbcak)
	...
	cclog("function GameFrameEngine:ctor(view,callbcak) ==> ")
...
function GameFrameEngine:setKindInfo(id,version)
	...
	cclog("function GameFrameEngine:setKindInfo(id,version) ==> ")
...
function GameFrameEngine:onInitData()
	cclog("function GameFrameEngine:onInitData() ==> ")
	--房间信息 以后转移
	self._wTableCount = 0
	self._wChairCount = 0
	self._wServerType = 0
...
function GameFrameEngine:setEnterAntiCheatRoom( bEnter )
	...
	cclog("function GameFrameEngine:setEnterAntiCheatRoom( bEnter ) ==> ")
	self.bEnterAntiCheatRoom = bEnter
end

--连接房间
function GameFrameEngine:onLogonRoom()
	cclog("function GameFrameEngine:onLogonRoom() ==> ")
	...
	cclog("登录房间："..self._roomInfo.szServerAddr.."#"..self._roomInfo.wServerPort)
...
function GameFrameEngine:onConnectCompeleted()
	cclog("function GameFrameEngine:onConnectCompeleted() ==> ")
...
function GameFrameEngine:onSocketEvent(main,sub,dataBuffer)
	cclog("function GameFrameEngine:onSocketEvent(main,sub,dataBuffer) ==> main： " .. main .."=sub: " .. sub)

	--登录信息
	if main == yl.MDM_GR_LOGON then--1
		self:onSocketLogonEvent(sub,dataBuffer)
	--配置信息
	elseif main == yl.MDM_GR_CONFIG then--2
		self:onSocketConfigEvent(sub,dataBuffer)
	--用户信息
	elseif main == yl.MDM_GR_USER then--3
		self:onSocketUserEvent(sub,dataBuffer)
	--状态信息
	elseif main == yl.MDM_GR_STATUS then--4
		self:onSocketStatusEvent(sub,dataBuffer)
	elseif main == yl.MDM_GF_FRAME then--100
		self:onSocketFrameEvent(sub,dataBuffer)
	elseif main == yl.MDM_GF_GAME then--200
		if self._viewFrame and self._viewFrame.onEventGameMessage then
			self._viewFrame:onEventGameMessage(sub,dataBuffer)
		end
	elseif main == game_cmd.MDM_GR_INSURE then--5
	...
	elseif main == game_cmd.MDM_GR_PROPERTY then -- 8
...
function GameFrameEngine:onSocketLogonEvent(sub,dataBuffer)
	cclog("function GameFrameEngine:onSocketLogonEvent(sub,dataBuffer) ==> " .. sub)
	...
	elseif sub == game_cmd.SUB_GR_LOGON_SUCCESS then
		local cmd_data = ExternalFun.read_netdata(game_cmd.CMD_GR_LogonSuccess, dataBuffer)
		-- dump(cmd_table, "CMD_GR_LogonSuccess", 4)
		G_CHECKCHEAT_RIGHT(cmd_data.dwUserRight, cmd_data.dwMasterRight)
	--登录失败
	elseif sub == game_cmd.SUB_GR_LOGON_FAILURE then	
		local errorCode = dataBuffer:readint()
		local msg = dataBuffer:readstring()
		cclog("登录房间失败:"..errorCode.."#"..msg)
		self:onCloseSocket()
...
function GameFrameEngine:onSocketLogonFinish()
	cclog("function GameFrameEngine:onSocketLogonFinish() ==> ")
	...
	cclog("GameFrameEngine:onSocketLogonFinish==2")
...
function GameFrameEngine:onSocketConfigEvent(sub,dataBuffer)
	...
	cclog("function GameFrameEngine:onSocketConfigEvent(sub,dataBuffer) ==> ")
	...
	if sub == yl.SUB_GR_CONFIG_SERVER then
		...
		cclog("房间配置[table:"..self._wTableCount.."][chair:"..self._wChairCount.."][type:"..self._wServerType.."][rule:"..self._dwServerRule.."]")
	--配置完成
...
function GameFrameEngine:OnGameAllowBankTake()
	...
	cclog("function GameFrameEngine:OnGameAllowBankTake() ==> ")
...
function GameFrameEngine:OnGameAllowBankSave()
	cclog("function GameFrameEngine:OnGameAllowBankSave() ==> ")
	return bit:_and(self._dwServerRule,0x00080000) ~= 0
end

function GameFrameEngine:IsAllowAvertCheatMode( )
	cclog("function GameFrameEngine:IsAllowAvertCheatMode() ==> ")
...
function GameFrameEngine:IsAllowPlazzScoreChange()
	...
	cclog("function GameFrameEngine:IsAllowPlazzScoreChange() ==> ")
...
function GameFrameEngine:OnGameAllowBankTransfer()
	...
	cclog("function GameFrameEngine:OnGameAllowBankTransfer() ==> ")
...
function GameFrameEngine:onSocketUserEvent(sub,dataBuffer)
	cclog("function GameFrameEngine:onSocketUserEvent(sub,dataBuffer) ==> " .. sub)
	...
	if sub == game_cmd.SUB_GR_USER_WAIT_DISTRIBUTE then
		...
		cclog("正在进行分组,请稍后...")
	--用户进入
	elseif sub == yl.SUB_GR_USER_ENTER then -- 100
		self:onSocketUserEnter(dataBuffer)
	--用户积分
	elseif sub == yl.SUB_GR_USER_SCORE then --101
		self:onSocketUserScore(dataBuffer)
	--用户状态
	elseif sub == yl.SUB_GR_USER_STATUS then -- 102
		self:onSocketUserStatus(dataBuffer)
	--请求失败
	elseif sub == yl.SUB_GR_REQUEST_FAILURE then -- 103
		self:onSocketReQuestFailure(dataBuffer)
	end
end
--用户进入
function GameFrameEngine:onSocketUserEnter(dataBuffer)
	cclog("function GameFrameEngine:onSocketUserEnter(dataBuffer) ==> ")
	...
	if not self._UserList[GlobalUserItem.dwUserID]  then
		if	bMySelfInfo == false then
			cclog("还未有自己信息，不处理其他用户信息")
			return
		end
	else 
		if bMySelfInfo == true then
			cclog("已有自己信息，不再次处理自己信息")
			return
		end
	end
	...
	cclog("useritem.cbGender===>" .. userItem.cbGender)
	...
	cclog("GameFrameEngine enter ==> ", userItem.szNickName, userItem.dwIpAddress, userItem.dwDistance)
...
function GameFrameEngine:onSocketUserScore(dataBuffer)
	...
	cclog("function GameFrameEngine:onSocketUserScore(dataBuffer) ==> ")
	...
	if  item ~= nil then
		...
		cclog("更新用户["..dwUserID.."]["..item.szNickName.."]["..item.lScore.."]")
...
function GameFrameEngine:onSocketUserStatus(dataBuffer)
	cclog("function GameFrameEngine:onSocketUserStatus(dataBuffer) ==> ")
	...
	if  bMySelfInfo == true then
		...
		elseif newstatus.wTableID ~= yl.INVALID_TABLE and self.bChangeDesk == true then
			...
			cclog("自己新状态:"..newstatus.cbUserStatus)
			if self._viewFrame and self._viewFrame.onEventUserStatus then
				self._viewFrame:onEventUserStatus(useritem,newstatus,oldstatus)
			end
		end 
...
function GameFrameEngine:onSocketReQuestFailure(dataBuffer)
	cclog("function GameFrameEngine:onSocketReQuestFailure(dataBuffer) ==> ")
	...
	if  self._viewFrame and self._viewFrame.onReQueryFailure then
		self._viewFrame:onReQueryFailure(cmdtable.lErrorCode,cmdtable.szDescribeString)
 		...
 		cclog("not viewframe or not onReQueryFailure")
	end
...
function GameFrameEngine:onSocketStatusEvent(sub,dataBuffer)
	cclog("function GameFrameEngine:onSocketStatusEvent(sub,dataBuffer) ==> ")

	if sub == yl.SUB_GR_TABLE_INFO then --桌子信息 --100
		cclog("SUB_GR_TABLE_INFO")
	...
	elseif sub == yl.SUB_GR_TABLE_STATUS then	--桌子状态--101
		...
		cclog("SUB_GR_TABLE_STATUS ==> " .. wTableID--[[ .. " ==> " .. self._tableStatus[wTableID].cbPlayStatus]])
...
function GameFrameEngine:onSocketFrameEvent(sub,dataBuffer)
	cclog("function GameFrameEngine:onSocketFrameEvent(sub,dataBuffer) ==> " .. sub)
	--游戏状态
	if sub == yl.SUB_GF_GAME_STATUS then--100
		self._cbGameStatus = dataBuffer:readword()
		self._cbAllowLookon = dataBuffer:readword()
	--游戏场景
	elseif sub == yl.SUB_GF_GAME_SCENE then--101
		if self._viewFrame and self._viewFrame.onEventGameScene then
			...
			cclog("game scene did not respon")
			if not self._viewFrame then
				cclog ("viewframe is nl")
			else
				cclog("onEventGameScene is ni viewframe is"..self._viewFrame:getTag())
			end
		end
	--系统消息
	elseif sub == yl.SUB_GF_SYSTEM_MESSAGE then--200
		self:onSocketSystemMessage(dataBuffer)
	--动作消息
	elseif sub == yl.SUB_GF_ACTION_MESSAGE then--201
	...
	elseif sub == game_cmd.SUB_GF_USER_CHAT then
		...
		if nil ~= self._viewFrame and nil ~= self._viewFrame.onUserChat then
			self._viewFrame:onUserChat(chat,useritem.wChairID, useritem.cbGender)
		end
	--用户表情
...
function GameFrameEngine:onSocketActionMessage(dataBuffer)
	...
	cclog("function GameFrameEngine:onSocketActionMessage(dataBuffer) ==> ")

	local wType = dataBuffer:readword()
	local wLength = dataBuffer:readword()
	local nButtonType = dataBuffer:readint()
	local szString = dataBuffer:readstring()
	cclog("系统动作#"..wType.."#"..szString)
...
function GameFrameEngine:onUpDataTableUser(tableid,chairid,useritem)
	cclog("function GameFrameEngine:onUpDataTableUser(tableid,chairid,useritem) ==> ")
...
function GameFrameEngine:getTableUserItem(tableid,chairid)
	cclog("function GameFrameEngine:getTableUserItem(tableid,chairid) ==> ")
...
function GameFrameEngine:getTableInfo(index)
	...
	cclog("function GameFrameEngine:getTableInfo(index) ==> ")
...
function GameFrameEngine:GetMeUserItem()
	cclog("function GameFrameEngine:GetMeUserItem() ==> ")
...
function GameFrameEngine:GetGameStatus()
	...
	cclog("function GameFrameEngine:GetGameStatus() ==> ")
...
function GameFrameEngine:SetGameStatus(cbGameStatus)
	cclog("function GameFrameEngine:SetGameStatus(cbGameStatus) ==> " .. cbGameStatus)
...
function GameFrameEngine:GetTableID()
	...
	cclog("function GameFrameEngine:GetTableID() ==> ")
...
function GameFrameEngine:GetChairID()
	...
	cclog("function GameFrameEngine:GetChairID() ==> ")
...
function GameFrameEngine:onRemoveUser(dwUserID)
	...
	cclog("function  GameFrameEngine:onRemoveUser(dwUserID) ==> ")
...
function GameFrameEngine:SitDown(table ,chair,password)
	cclog("function GameFrameEngine:SitDown(table ,chair,password) ==> ")
...
function GameFrameEngine:QueryUserInfo(table ,chair)
	...
	cclog("function GameFrameEngine:QueryUserInfo(table ,chair) ==> ")
...
function GameFrameEngine:QueryChangeDesk()
	...
	cclog("function GameFrameEngine:QueryChangeDesk() ==> ")
...
function GameFrameEngine:StandUp(bForce)
	...
	cclog("function GameFrameEngine:StandUp(bForce) ==> ")
...
function GameFrameEngine:SendUserReady(dataBuffer)
	cclog("function GameFrameEngine:SendUserReady(dataBuffer) ==> ")
...
function GameFrameEngine:SendGameOption()
	...
	cclog("function GameFrameEngine:SendGameOption() ==> ")
...
function GameFrameEngine:SendEncrypt(pass)
	...
	cclog("function GameFrameEngine:SendEncrypt(pass) ==> ")

	local passlen = string.len(pass) * 2 --14--(ExternalFun.stringLen(pass)) * 2
	cclog("passlen ==> " .. passlen)
...
function GameFrameEngine:sendTextChat( msg, tagetUser , color)
	...
	cclog("function GameFrameEngine:sendTextChat( msg, tagetUser , color) ==> ")
...
function GameFrameEngine:sendBrowChat( idx, tagetUser )
	...
	cclog("function GameFrameEngine:sendBrowChat(idx, tagetUser) ==> ")
...
function GameFrameEngine:OnResetGameEngine()
	...
	cclog("function GameFrameEngine:OnResetGameEngine() ==> ")
...
function GameFrameEngine:popVocieMsg()
	...
	cclog("function GameFrameEngine:popVocieMsg() ==> ")
...
function GameFrameEngine:setPlayingVoice( bPlaying )
	...
	cclog("function GameFrameEngine:setPlayingVoice( bPlaying ) ==> ")
...
function GameFrameEngine:clearVoiceQueue()
	...
	cclog("function GameFrameEngine:clearVoiceQueue() ==> ")
...
function GameFrameEngine:onSysMessage(main, sub, pData)
	cclog("GameFrameEngine:onSysMessage===>" .. sub)
	if self._viewFrame and self._viewFrame.onEventGameScene then
		self._viewFrame:onSysMessage(pData)
	else
		cclog("game scene did not respon")
		if not self._viewFrame then
			cclog ("viewframe is nl")
