...
PWD_DEFAULT = "NONE"
-- 账户文件
USER_GAMECONFIG = device.writablePath .. "user_gameconfig.plist"
------------------------------------------------------------------
...
function GlobalUserItem.reSetData()
	...
	cclog("function GlobalUserItem.reSetData() ==> ")
...
--获取pwd
function GlobalUserItem.getPwd_MD5()
	--没有密码的发动态密码，如游客和微信
	if GlobalUserItem.szPassword == PWD_DEFAULT then
		return GlobalUserItem.szDynamicPass
	else
		return md5(GlobalUserItem.szPassword)
	end
end

--读取配置
function GlobalUserItem.LoadData()
	cclog("function GlobalUserItem.LoadData() ==> begin")
	...
	cclog("function GlobalUserItem.LoadData() ==> 2")
	...
	cclog("function GlobalUserItem.LoadData() ==3 ")
	--账号密码
	cclog("function GlobalUserItem.LoadData() ==USER_GAMECONFIG: " .. USER_GAMECONFIG)
	local tmpInfo = readByDecrypt(USER_GAMECONFIG,"code_1")
	cclog("function GlobalUserItem.LoadData() ==tmpInfo： " .. tmpInfo)
	--检验
	...
	if tmpInfo ~= nil then
		...
		tmpInfo = readByDecrypt(USER_GAMECONFIG,"code_2")
		--检验
		if tmpInfo ~= nil and #tmpInfo >32 then
			local md5Test = string.sub(tmpInfo,1,32)
			tmpInfo = string.sub(tmpInfo,33,#tmpInfo)
		...
		if tmpInfo ~= nil then
			cclog("GlobalUserItem.LoadData===tmpInfo: " .. tmpInfo)
	...
	cclog("function GlobalUserItem.LoadData() ==> end")
end
...
function GlobalUserItem.onSaveAccountConfig()
	cclog("function GlobalUserItem.onSaveAccountConfig() ==> begin==USER_GAMECONFIG: " .. USER_GAMECONFIG )
	...
	cclog("function GlobalUserItem.onSaveAccountConfig()==code_1: " .. szSaveInfo)
	saveByEncrypt(USER_GAMECONFIG,"code_1",szSaveInfo)
	...
	cclog("function GlobalUserItem.onSaveAccountConfig()==code_2: " .. szSaveInfo)
	saveByEncrypt(USER_GAMECONFIG,"code_2",szSaveInfo)

	cc.UserDefault:getInstance():setBoolForKey("autologon",GlobalUserItem.bAutoLogon)

	cclog("function GlobalUserItem.onSaveAccountConfig() ==> end")
end

--查询房间数目(普通房間)
function GlobalUserItem.GetGameRoomCount(nKindID)
	cclog("function GlobalUserItem.GetGameRoomCount(nKindID) ==> ")
	...
	if not nKindID then
		...
		cclog("GlobalUserItem.GetGameRoomCount==checkKind: " .. checkKind)
	end
	cclog("GlobalUserItem.GetGameRoomCount====" .. #GlobalUserItem.roomlist)
	for i = 1,#GlobalUserItem.roomlist do
		local list = GlobalUserItem.roomlist[i]
		local tmpKindID = tonumber(list[1])
		cclog("GlobalUserItem.GetGameRoomCount===tmpKindID: " .. tmpKindID)
		if tmpKindID == checkKind then
			if not list[2] then
				cclog("GlobalUserItem.GetGameRoomCount, list[2]serveritem is Null")
				break
			end
			local nCount = 0
			local serverList = list[2]
			for k,v in pairs(serverList) do
				cclog("GlobalUserItem.GetGameRoomCount==v.wServerType： " .. v.wServerType .. "=yl.GAME_GENRE_PERSONAL: " .. yl.GAME_GENRE_PERSONAL)
				if v.wServerType ~= yl.GAME_GENRE_PERSONAL then
					nCount = nCount + 1
					cclog("GlobalUserItem.GetGameRoomCount==nCount1: " .. nCount)
				end				
			end
			cclog("GlobalUserItem.GetGameRoomCount==return nCount2: " .. nCount)
			return nCount
		end
	end
	return 0
...
function GlobalUserItem.GetGameRoomInfo(wServerID, nKindID)
	cclog("function GlobalUserItem.GetGameRoomInfo(wServerID, nKindID) ==> ")
	...
	for i = 1,#GlobalUserItem.roomlist do
		...
		if tonumber(list[1]) == checkKind then
			if not list[2] then
				cclog("GlobalUserItem.GetGameRoomInfo, list[2]serveritem is Null")
				break
			end
			local nCount = 0
			local serverList = list[2]
...
function GlobalUserItem.setShakeAble(able)
	...
	cclog("function GlobalUserItem.setShakeAble(able) ==> ")
...
function GlobalUserItem.setSoundAble(able)
	...
	cclog("function GlobalUserItem.setSoundAble(able) ==> ")
...
function GlobalUserItem.setVoiceAble(able)
	...
	cclog("function GlobalUserItem.setVoiceAble(able) ==> ")
...
function GlobalUserItem.setMusicVolume(music)
	...
	cclog("function GlobalUserItem.setMusicVolume(music) ==> ")
...
function GlobalUserItem.setEffectsVolume(sound)
	...
	cclog("function GlobalUserItem.setEffectsVolume(sound) ==> ")
...
function GlobalUserItem.GetRoomInfo(index,nKindID)
	cclog("function GlobalUserItem.GetRoomInfo(index,nKindID) ==> ")
...
function GlobalUserItem.normalRoomIndex(nKindID)
	...
	cclog("function GlobalUserItem.normalRoomIndex(nKindID) ==> ")
...
function GlobalUserItem.onLoadData(pData)
	cclog("function GlobalUserItem.onLoadData(pData) ==> ")

	if pData == nil then
		cclog("GlobalUserItem-LoadData-null")
		return
	end
	--登录时间
	...
	cclog("GlobalUserItem.onLoadData===userid: " .. GlobalUserItem.dwUserID)
	...
	GlobalUserItem.szDynamicPass= pData:readstring(33) --动态密码
	...
	cclog("lock server " .. GlobalUserItem.dwLockServerID)
	cclog("lock kind " .. GlobalUserItem.dwLockKindID)

	local curlen = pData:getcurlen()
	local datalen = pData:getlen()

	cclog("*** curlen-"..curlen)
	cclog("*** datalen-"..datalen)
...
function GlobalUserItem:readScore(dataBuffer)
	...
	cclog("function GlobalUserItem:readScore(dataBuffer) ==> ")
...
function GlobalUserItem:getDateNumber(datestr)
	...
	cclog("function GlobalUserItem:getDateNumber(datestr) ==> ")
...
function GlobalUserItem.isAntiCheat()
	...
	cclog("function GlobalUserItem.isAntiCheat() ==> ")
...
function GlobalUserItem.isAntiCheatValid(userid)
	...
	cclog("function GlobalUserItem.isAntiCheatValid(userid) ==> ")
...
function GlobalUserItem.todayCheck(date)
	...
	cclog("function GlobalUserItem.todayCheck(date) ==> ")
...
function GlobalUserItem.setTodayFirstAction(key, value)
	...
	cclog("function GlobalUserItem.setTodayFirstAction(key, value) ==> ")
...
function GlobalUserItem.isFirstCheckIn()
	cclog("function GlobalUserItem.isFirstCheckIn() ==> ")
...
function GlobalUserItem.setTodayCheckIn()
	cclog("function GlobalUserItem.setTodayCheckIn() ==> ")
...
function GlobalUserItem.isFirstPay()
	...
	cclog("function GlobalUserItem.isFirstPay() ==> ")
...
function GlobalUserItem.setTodayPay()
	...
	cclog("function GlobalUserItem.setTodayPay() ==> ")
...
function GlobalUserItem.isFirstShare()
	...
	cclog("function GlobalUserItem.isFirstShare() ==> ")
...
function GlobalUserItem.setTodayShare()
	...
	cclog("function GlobalUserItem.setTodayShare() ==> ")
...
function GlobalUserItem.isFirstTable()
	...
	cclog("function GlobalUserItem.isFirstTable() ==> ")
...
function GlobalUserItem.setTodayTable()
	...
	cclog("function GlobalUserItem.setTodayTable() ==> ")
...
function GlobalUserItem.isShowAdNotice()
	cclog("function GlobalUserItem.isShowAdNotice() ==> ")
...
function GlobalUserItem.setTodayNoAdNotice( noAds )
	...
	cclog("function GlobalUserItem.setTodayNoAdNotice( noAds ) ==> ")
...
function GlobalUserItem.isAngentAccount(nottip)
	...
	cclog("function GlobalUserItem.isAngentAccount(nottip) ==> ")
...
function GlobalUserItem.setBindingAccount()
	...
	cclog("function GlobalUserItem.setBindingAccount() ==> ")
...
function GlobalUserItem.getBindingAccount()
	cclog("function GlobalUserItem.getBindingAccount() ==> ")
...
function GlobalUserItem.notEditAble(nottip)
	...
	cclog("function GlobalUserItem.notEditAble(nottip) ==> ")
...
function GlobalUserItem.noCoordinateData()
	...
	cclog("function GlobalUserItem.noCoordinateData() ==> ")
