...
function BaseFrame:ctor(view,callback)
	...
	cclog("function BaseFrame:ctor(view,callback) ==>")
...
function BaseFrame:setCallBack(callback)
	...
	cclog("function BaseFrame:setCallBack(callback) ==>")
...
function BaseFrame:onSocketError(pData)
	cclog("function BaseFrame:onSocketError(pData) ==>begin")
	...
	cclog("function BaseFrame:onSocketError(pData) ==>end")
end

--启动网络
function  BaseFrame:onCreateSocket(szUrl,nPort)
	cclog("function  BaseFrame:onCreateSocket(szUrl,nPort) ==>")
	...
	if nil == szUrl then
		self._szServerUrl = yl.LOGONSERVER
		self._nServerPort = yl.LOGONPORT
	else
...
function BaseFrame:onSocketCallBack(pData)
	cclog("function BaseFrame:onSocketCallBack(pData) ==>")
	...
	elseif main == yl.MDM_CM_SYSTEM then
		cclog("BaseFrame:onSocketCallBack===>系统消息")
		if sub == yl.SUB_CM_SYSTEM_MESSAGE then
		   self:onSysMessage(main, sub, pData)
		end
	else
...
function BaseFrame:onCloseSocket()
	cclog("function BaseFrame:onCloseSocket() ==>")
...
function BaseFrame:sendSocketData(pData)
	cclog("function BaseFrame:sendSocketData(pData) ==>")
...
function BaseFrame:onConnectCompeleted()
	cclog("warn BaseFrame-onConnectResult-"..result)
end

--网络信息(短连接)
function BaseFrame:onSocketEvent(main,sub,pData)
	cclog("warn BaseFrame-onSocketData-"..main.."-"..sub)
end

--网络消息(长连接)
function BaseFrame:onGameSocketEvent(main,sub,pData)
	cclog("warn BaseFrame-onGameSocketEvent-"..main.."-"..sub)
end

--系统消息
function BaseFrame:onSysMessage(main,sub,pData)
	cclog("warn BaseFrame-onSysMessage-"..main.."-"..sub)
end
