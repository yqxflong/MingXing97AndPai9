...
function LogonFrame:ctor(view,callback)
	...
	cclog("function LogonFrame:ctor(view,callback) ==> ")
...
function LogonFrame:onConnectCompeleted()
	cclog("function LogonFrame:onConnectCompeleted() ==> ")
...
function LogonFrame:onSocketEvent(main,sub,pData)
	cclog("function LogonFrame:onSocketEvent(main,sub,pData) ==> ")
...
function LogonFrame:onSubLogonEvent(sub,pData)
	cclog("function LogonFrame:onSubLogonEvent(sub,pData) ==> ")
	...
	elseif sub == yl.SUB_MB_UPDATE_NOTIFY then
		...
		cclog("update_notify:"..cbMustUpdate.."#"..cbAdviceUpdate.."#"..dwCurrentVersion)
		local tmpV = appdf.ValuetoVersion(dwCurrentVersion)
		cclog(tmpV.p.."."..tmpV.m.."."..tmpV.s.."."..tmpV.b)
		self:onCloseSocket()
		if nil ~= self._callBack then
...
function LogonFrame:onRoomListEvent(sub,pData)
	cclog("function LogonFrame:onRoomListEvent(sub,pData) ==> sub: " .. sub)
...
function LogonFrame:onSubRoomListInfo(pData)
	cclog("function LogonFrame:onSubRoomListInfo(pData) ==> ")

	--计算房间数目
	local len = pData:getlen()
	cclog("=============== onSubRoomListInfo ================")
	cclog("onSubRoomListInfo:"..len)
	if (len - math.floor(len/yl.LEN_GAME_SERVER_ITEM)*yl.LEN_GAME_SERVER_ITEM) ~= 0 then
		cclog("roomlist_len_error"..len)
		return
	end
	local itemcount =  math.floor(len/yl.LEN_GAME_SERVER_ITEM)
	cclog("=============== onSubRoomListInfo ================itemcount: " .. itemcount)
...
function LogonFrame:onSubAngentListInfo(pData)
	cclog("function LogonFrame:onSubAngentListInfo(pData) ==> ")

	self.m_angentServerList = {}
	--计算数目
	local len = pData:getlen()
	cclog("=============== onSubAngentListInfo ================")
	cclog("onSubAngentListInfo:"..len)
	if (len - math.floor(len/4)*4) ~= 0 then
		cclog("angentlist_len_error"..len)
		return
	end
	cclog("=============== onSubAngentListInfo ================")
...
function LogonFrame:onLogonByAccount(szAccount,szPassword)
	cclog("function LogonFrame:onLogonByAccount(szAccount,szPassword) ==> ")
...
function LogonFrame:onLogonByVisitor()
	cclog("function LogonFrame:onLogonByVisitor() ==> ")
	--记录模式	
	self._logonMode = 2
	if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then
		self._callBack(-1,"建立连接失败！")
...
function LogonFrame:onLoginByThirdParty(szAccount, szNick, cbgender, platform)
	cclog("function LogonFrame:onLoginByThirdParty(szAccount, szNick, cbgender, platform) ==> ")
	--数据保存
...
function LogonFrame:onRegister(szAccount,szPassword,cbGender,szSpreader)
	...
	cclog("function LogonFrame:onRegister(szAccount,szPassword,cbGender,szSpreader) ==> ")
	--数据保存
...
function LogonFrame:sendLogon()
	...
	cclog("function LogonFrame:sendLogon() ==> ")
	...
	--cclog("send logon:account-"..self._szAccount.." password-"..self._szPassword)
...
function LogonFrame:sendVisitor()
	cclog("function LogonFrame:sendVisitor() ==> ")
...
function LogonFrame:sendThirdPartyLogin( )
	...
	cclog("function LogonFrame:sendThirdPartyLogin() ==> ")
...
function LogonFrame:sendRegister()
	...
	cclog("function LogonFrame:sendRegister() ==> ")
