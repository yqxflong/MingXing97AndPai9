...
--好友文件存放路径
local FRIEND_PATH = device.writablePath .. "r__friend_record/"
--我的好友
...
function FriendMgr:getInstance()
	...
	cclog("function FriendMgr:getInstance() ==> ")
...
function FriendMgr:create()
	...
	cclog("function FriendMgr:create() ==> ")
...
function FriendMgr:init()
	...
	--创建私人房间后好友邀请列表
	self.m_inviteListAfterCreatePriRoom = {}


	--房间管理玩家回调列表
	self.mCallBacks_RoomManagementPlayerStatus = {}
end

----------------------------注册回调--------------------------------
--注册房间管理玩家状态回调
function FriendMgr:registerRoomManagementPlayerStatusCallBack(k, _callback)
	self.mCallBacks_RoomManagementPlayerStatus[k] = _callback
end
function FriendMgr:unregisterRoomManagementPlayerStatusCallBack(k, _callback)
	self.mCallBacks_RoomManagementPlayerStatus[k] = nil
end
function FriendMgr:clearRoomManagementPlayerStatusCallBacks()
	self.mCallBacks_RoomManagementPlayerStatus = {}
end
----------------------------注册回调--------------------------------
...
---------------------------about 邀请好友列表操作------------------------------
function FriendMgr:insertInviteList(uid)
	cclog("FriendMgr:insertInviteList====>")
	self.m_inviteListAfterCreatePriRoom[uid]  = 1
end

function FriendMgr:removeInviteList(uid)
	cclog("FriendMgr:removeInviteList====>")
	self.m_inviteListAfterCreatePriRoom[uid]  = nil
end

function FriendMgr:clearInviteList()
	self.m_inviteListAfterCreatePriRoom = {}
end

function FriendMgr:isFriendInInviteList(uid)
	if self.m_inviteListAfterCreatePriRoom[uid] == 1 then
		cclog("FriendMgr:isFriendInInviteList==>1111")
		return true
	else
		cclog("FriendMgr:isFriendInInviteList==>2222")
		return false
	end
end
---------------------------about 邀请好友列表操作------------------------------
...
function FriendMgr:registerNotifyList()
	...
	cclog("function FriendMgr:registerNotifyList() ==> ")
...
function FriendMgr:setCallBackDelegate(delegate,callback)
	...
	cclog("function FriendMgr:setCallBackDelegate(delegate,callback) ==> ")
	--self.m_Delegate = delegate
	--self.m_CallBack = callback;
end

function FriendMgr:setViewLayer(layer)
	cclog("function FriendMgr:setViewLayer(layer) ==> ")
	self.m_viewLayer = layer
end

--通知消息回调
function FriendMgr:onNotify(msg)
	cclog("function FriendMgr:onNotify(msg) ==> ")
...
function FriendMgr:onSocketCallBack(pData)
	cclog("function FriendMgr:onSocketCallBack(pData) ==> ")
...
function FriendMgr:onConnectCompeleted()
	...
	cclog("function FriendMgr:onConnectCompeleted() ==> ")
...
function FriendMgr:getLocationInfo()
	...
	cclog("function FriendMgr:getLocationInfo() ==> ")
...
function FriendMgr:updateLocation()
	...
	cclog("function FriendMgr:updateLocation() ==> ")
...
function FriendMgr:setWaitTime( var )
	...
	cclog("function FriendMgr:setWaitTime(var) ==> ")
...
function FriendMgr:onSocketError(pData)
	...
	cclog("function FriendMgr:onSocketError(pData) ==> ")
...
function FriendMgr:onSocketEvent(main,sub,pData)
	cclog("FriendMgr:onSocketEvent====>:"..main.."#"..sub) 
	--作为消息通知的参数
	local netdata = nil
	self:dismissPopWait()
	...
	if main == chat_cmd.MDM_GC_LOGON  then	 				--用户信息
		...
		elseif sub == chat_cmd.SUB_GC_LOGON_SUCCESS then 	-- 登陆成功
			cclog("FriendMgr:onSocketEvent===>登陆成功")
			self:updateLocation()
		end
...
function FriendMgr:onFriendMessage(sub,pData)
	cclog("function FriendMgr:onFriendMessage(sub,pData) ==> " .. sub)
	...
	if sub == chat_cmd.SUB_GC_USER_STATUS_NOTIFY then  			--好友状态(上下线)
		...
		for k,v in pairs(self.m_FriendTab) do
			if v.dwUserID == userStatus.dwUserID then
				cclog("好友状态 ==> " .. userStatus.cbMainStatus)		
	...
	elseif sub == chat_cmd.SUB_GC_GAME_STATUS_NOTIFY then  		--游戏状态							
		...
		for k,v in pairs(self.m_FriendTab) do
			if v.dwUserID == userStatus.dwUserID then
				cclog("游戏状态 ==> " .. userStatus.cbGameStatus)
		...
		if nil ~= userInfo then
			if nil ~= self.m_viewLayer and nil ~= self.m_viewLayer.onRefreshFriendState then
				self.m_viewLayer:onRefreshFriendState(userInfo, true)
			end
		end		
	elseif sub == chat_cmd.SUB_GC_PERSON_STATUS_NOTIFY then     --房间管理玩家状态变更
		local cmd_table = ExternalFun.read_netdata(chat_cmd.CMD_GC_PersonUserStatusNotify, pData)
		for k, callback in pairs(self.mCallBacks_RoomManagementPlayerStatus) do
			callback(1, cmd_table)
		end
	elseif sub == chat_cmd.SUB_GC_PERSONAL_ROOM_OWNER_RESULT then  --房间管理信息结果
		local cmd_table = ExternalFun.read_netdata(chat_cmd.CMD_GC_RefreshPersonRoomOwner, pData)
		for k, callback in pairs(self.mCallBacks_RoomManagementPlayerStatus) do
			callback(2, cmd_table)
		end
	elseif sub == chat_cmd.SUB_GC_USER_CHAT_NOTIFY  then 		--聊天通知
		...
		if nil ~= self.m_viewLayer and nil ~= self.m_viewLayer.onMessageNotify then
			self.m_viewLayer:onMessageNotify(notify)
		end
	elseif sub == chat_cmd.SUB_GC_APPLYFOR_NOTIFY then 			--申请通知
		...
		if nil ~= self.m_viewLayer and nil ~= self.m_viewLayer.onUpdateNotifyList then
			self.m_viewLayer:onUpdateNotifyList()
		end	
	...
	elseif sub == chat_cmd.SUB_GC_SEARCH_USER_RESULT then 		--查找通知/查找结果
		cclog("FriendMgr:onFriendMessag===>查找通知/查找结果")
		...
		if nil ~= self.m_viewLayer and nil ~= self.m_viewLayer.onSearchResult then
			self.m_viewLayer:onSearchResult(userTab)
		end	
  ...
  --               :addTo(runScene)
	...
	elseif sub == chat_cmd.SUB_GC_QUERY_NEARUSER_ECHO 
		...
		cclog(data_tab.szDescribeString)
		-- self:dismissPopWait()
...
function FriendMgr:showEcho(sub,pData)
	...
	cclog("function FriendMgr:showEcho(sub,pData) ==> ")
...
function FriendMgr:showFriendEcho(sub, pData)
	cclog("function FriendMgr:showFriendEcho(sub, pData) ==> " .. sub)
...
function FriendMgr:contentAndLogin()
	cclog("function FriendMgr:contentAndLogin() ==> ")

	self.m_Socket = CClientSocket:createSocket(function(pData)
		self:onSocketCallBack(pData)
	end)

	self.m_Socket:connectSocket(yl.LOGONSERVER, yl.FRIENDPORT, yl.VALIDATE)
end

--好友登陆
function FriendMgr:login()
	cclog("function FriendMgr:login() ==> ")
	...
	friendData:pushstring(GlobalUserItem.getPwd_MD5(),yl.LEN_PASSWORD);
	friendData:pushstring("手机型号",chat_cmd.LEN_PHONE_MODE);

	if not self:sendSocketData(friendData) then
		cclog("FriendMgr:login 登录好友系统失败！！！")
		return
	end
...
function FriendMgr:sendMessageFriend(userTab)
	cclog("function FriendMgr:sendMessageFriend(userTab) ==> ")
...
function FriendMgr:sendAddFriend(userTab,func)
	cclog("function FriendMgr:sendAddFriend(userTab,func) ==> ")
	local runScene = cc.Director:getInstance():getRunningScene()
	---
	...
	if not self:sendSocketData(addFriendData) then
		...
		func(true);
		showToast(runScene, "已发送好友申请，请等待对方验证", 1)
	end	
end
...
function FriendMgr:sendRespondFriend(userTab,notifyId)
	...
	cclog("function FriendMgr:sendRespondFriend(userTab,notifyId) ==> ")
	...
	if not self:sendSocketData(respondData) then
		...
		--[[
			Johny
			改为及时删除，只提示未读消息，不存储已读
			<self:markFriendNotifyRead(notifyId);>
		]]
		self:deleteFriendNotify(notifyId)
		if nil ~= self.m_viewLayer and nil ~= self.m_viewLayer.onUpdateNotifyList then	
			self.m_viewLayer:onUpdateNotifyList()
		end
	end
...
function FriendMgr:sendSearchFriend(dwuserid)
	cclog("function FriendMgr:sendSearchFriend(dwuserid) ==> 查找: " .. dwuserid)
	local searchData = ExternalFun.create_netdata(chat_cmd.CMD_GC_SearchByGameID)
	...
	if not self:sendSocketData(searchData) then
		cclog("FriendMgr:sendSearchFriend===>发送查找好友失败！");
	end	
end

--邀请游戏
function FriendMgr:sendInviteGame(msgTab)
	cclog("function FriendMgr:sendInviteGame(msgTab) ==> ")
...
function FriendMgr:sendInvitePrivateGame(msgTab)
	cclog("function FriendMgr:sendInvitePrivateGame(msgTab) ==> ")
...
-- 创建私房成功后邀请选中的好友
function FriendMgr:sendInvitePrivateGameAfterCreatePrivateRoom()
	local m_tabDetail = {}
	...
	--遍历发送邀请
	for uid,v in pairs(self.m_inviteListAfterCreatePriRoom) do
		local frienddata = self:getFriendByID(uid)
...
function FriendMgr:sendTrupmet( msgTab )
	cclog("function FriendMgr:sendTrupmet( msgTab ) ==> ")
	...
	if not self:sendSocketData(sendMsgData) then
		cclog("FriendMgr:sendTrupmet====>发送喇叭失败！");
		sendMsgData:retain()
		table.insert(self.m_sendCache, sendMsgData)
	end
end
...
function FriendMgr:sendDeleteFriend(friendid, groupid)
	cclog("function FriendMgr:sendDeleteFriend(friendid, groupid) ==> ")
...
function FriendMgr:sendUpdateCoordinate(lLongitude, lLatitude)
	...
	cclog("function FriendMgr:sendUpdateCoordinate(lLongitude, lLatitude) ==> ")
...
function FriendMgr:queryNearUser()
	...
	cclog("function FriendMgr:queryNearUser() ==> ")
...
function FriendMgr:sendQueryNearUser()
	...
	cclog("function FriendMgr:sendQueryNearUser() ==> ")
...
function FriendMgr:sendQueryUserLocation( dwTargetUserID )
	...
	cclog("function FriendMgr:sendQueryUserLocation( dwTargetUserID ) ==> ")
...
function FriendMgr:sendShareMessage(msgTab)
	...
	cclog("function FriendMgr:sendShareMessage(msgTab) ==> ")
...
function FriendMgr:sendSocketData(pData)
	cclog("function FriendMgr:sendSocketData(pData) ==> ")
...
function FriendMgr:onCloseSocket()
	...
	cclog("function FriendMgr:onCloseSocket() ==> ")
	...
	if self.m_nAutoLoginCount > 0 then
		...
		cclog("auto connect ==> 0")
	end	
end

--判断是否有网络连接
function FriendMgr:isConnected()
	cclog("function FriendMgr:isConnected() ==> ")
...
function FriendMgr:reSetAndLogin( )
	...
	cclog("function FriendMgr:reSetAndLogin() ==> ")
	--聊天缓存
	self.m_tabChatCache[GlobalUserItem.dwUserID] = {}
	--通知缓存
	self.m_tabNoticeCache[GlobalUserItem.dwUserID] = {}
...
function FriendMgr:reSetAndDisconnect(  )
	...
	cclog("function FriendMgr:reSetAndDisconnect() ==> ")
...
function FriendMgr:popCacheMessage()
	cclog("function FriendMgr:popCacheMessage() ==> ")
	--处理未发送的缓存数据
	for k,v in pairs(self.m_sendCache) do
...
function FriendMgr:insertFriendNotify(Type,notifyTab)
	cclog("function FriendMgr:insertFriendNotify(Type,notifyTab) ==> ")
...
function FriendMgr:getFriendNotify()
	cclog("function FriendMgr:getFriendNotify() ==> ")
...
function FriendMgr:getUnReadNotify()
	cclog("function FriendMgr:getUnReadNotify() ==> ")
...
function FriendMgr:deleteFriendNotify(notifyId)
	cclog("function FriendMgr:deleteFriendNotify(notifyId) ==> ")
...
function FriendMgr:markFriendNotifyRead(notifyId)
	cclog("function FriendMgr:markFriendNotifyRead(notifyId) ==> ")
...
function FriendMgr:deleteFriend(index)
	...
	cclog("function FriendMgr:deleteFriend(index) ==> ")
...
function FriendMgr:getFriendList()
	...
	cclog("function FriendMgr:getFriendList() ==> ")
...
function FriendMgr:getFriendByID( dwID )
	...
	cclog("function FriendMgr:getFriendByID( dwID ) ==> ")
...
function FriendMgr:getFriendInfoByID(dwUserID)
	...
	cclog("function FriendMgr:getFriendInfoByID(dwUserID) ==> ")
...
function FriendMgr:insertUserMsg(userID, sendID, targetID, content, bImage)
	...
	cclog("function FriendMgr:insertUserMsg(userID, sendID, targetID, content, bImage) ==> ")
	...
	if cc.FileUtils:getInstance():isFileExist(filePath) then			
		...
		if true == ok then
			...
			if #dataTab > MAX_CHAT_CACHE then
				local msg = dataTab[1]
				if msg.bImage then
					cclog("FriendMgr: removeChatImage")
...
function FriendMgr:getUserRecordMsg(userID)
	...
	cclog("function FriendMgr:getUserRecordMsg(userID) ==> ")
...
function FriendMgr:showPopWait(msg)
	cclog("function FriendMgr:showPopWait(msg) ==> ")
...
function FriendMgr:dismissPopWait()
	cclog("function FriendMgr:dismissPopWait() ==> ")
...
function FriendMgr:downloadShareImage( imageurl )
	...
	cclog("function FriendMgr:downloadShareImage( imageurl ) ==> ")
