...
local privatemgr = "client/src/privatemode/plaza/src/models/PriRoom.lua"
...
appdf.req(appdf.EXTERNAL_SRC .. "SubGameUpdateAgent")
...
cc.exports.g_LuaErrorHandle = function ()
	...
	if isDebug() then
		...
		cclog("release return")
		return false
	end end

--加载配置
function LogonScene.onceExcute()
	cclog("function LogonScene.onceExcute() ==>1")

	local MultiPlatform = appdf.req(appdf.EXTERNAL_SRC .. "MultiPlatform")
	cclog("function LogonScene.onceExcute() ==>2")
	--文件日志
	LogAsset:getInstance():init(MultiPlatform:getInstance():getExtralDocPath(), true, true)
	cclog("function LogonScene.onceExcute() ==>3")
	--配置微信
	...
	cclog("function LogonScene.onceExcute() ==>4")
end
...
function LogonScene:onEnterTransitionFinish()
	cclog("function LogonScene:onEnterTransitionFinish() ==>")
...
function LogonScene:onExitTransitionStart()
	cclog("function LogonScene:onExitTransitionStart() ==>")
...
function LogonScene:onCreate()
	cclog("function LogonScene:onCreate() ==> ")
	local this = self
	...
	self._txtTips = cc.Label:createWithTTF("同步服务器信息中...", "fonts/yuanti_sc_light.ttf", 24)
...
--@进入游戏大厅
function LogonScene:enterClientScene()
	self:getApp():enterSceneEx(appdf.CLIENT_SRC.."plaza.views.ClientSceneNew","FADE",1)
end

--按钮事件
function LogonScene:onButtonClickedEvent(tag,ref)
	cclog("function LogonScene:onButtonClickedEvent(tag,ref) ==>")
...
function LogonScene:onShowLogon(bCheckAutoLogon)
	...
	cclog("function LogonScene:onShowLogon(bCheckAutoLogon) ==>")
...
function LogonScene:onShowRegister()
	cclog("function LogonScene:onShowRegister() ==>")
	--创建注册界面
	local registerview = RegisterView:create()
	registerview:addTo(self._backLayer)
end

--显示用户条款界面
function LogonScene:onShowService()
	cclog("function LogonScene:onShowService() ==>")
...
function LogonScene:onLogon(szAccount,szPassword,bSave,bAuto)
	...
	cclog("function LogonScene:onLogon(szAccount,szPassword,bSave,bAuto) ==>")
...
function LogonScene:onVisitor()
	cclog("function LogonScene:onVisitor() ==>")
...
function LogonScene:thirdPartyLogin(plat)
	cclog("function LogonScene:thirdPartyLogin(plat) ==>")
	...
	local function loginCallBack ( param )
		cclog("LogonScene:thirdPartyLogin-loginCallBack==>" .. param)
		self:dismissPopWait()
		if type(param) == "string" and string.len(param) > 0 then
			...
			if ok and type(datatable) == "table" then
				cclog("LogonScene:thirdPartyLogin===>" .. json.encode(datatable))
				local account = datatable["unionid"] or ""
				local nick = datatable["screen_name"] or ""
				self._szHeadUrl = datatable["profile_image_url"] or ""
				self._szHeadUrl = string.gsub(self._szHeadUrl, "\\", "") 
				self._szHeadUrl = string.gsub(self._szHeadUrl, "https", "http") 
				cclog("LogonScene:thirdPartyLogin===>" .. self._szHeadUrl)
				if datatable["gender"] == "男" then
				    gender = 1
				else
					gender = 0
				end
...
function LogonScene:onRegister(szAccount,szPassword,szRePassword,bAgreement,szSpreader)
	...
	cclog("function LogonScene:onRegister(szAccount,szPassword,szRePassword,bAgreement,szSpreader) ==>")
...
function LogonScene:onLogonCallBack(result,message)
	cclog("function LogonScene:onLogonCallBack(result,message) ==>" .. result)
	...
	if result == 1 then --成功
		...
		if self._Operate == 2 then 					--游客登录
			...
			GlobalUserItem.szPassword = PWD_DEFAULT
...
function LogonScene:showPopWait()
	...
	cclog("function LogonScene:showPopWait() ==>")
...
function LogonScene:dismissPopWait()
	cclog("function LogonScene:dismissPopWait() ==>")
...
function LogonScene:arrangeGameList(serverList)
	cclog("function LogonScene:arrangeGameList(serverList) ==>")
