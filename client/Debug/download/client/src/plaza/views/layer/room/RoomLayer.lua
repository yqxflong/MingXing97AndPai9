...
function RoomLayer:ctor(frameEngine, scene, bQuickStart)
	cclog("function RoomLayer:ctor(frameEngine, scene, bQuickStart)  ==>")
...
function RoomLayer:onExitRoom()
	cclog("function RoomLayer:onExitRoom()  ==>")
...
function RoomLayer:showPopWait()
	cclog("function RoomLayer:showPopWait()  ==>")
...
function RoomLayer:dismissPopWait()
	cclog("function RoomLayer:dismissPopWait()  ==>")
...
function RoomLayer:onButtonClickedEvent(tag,ref)
	cclog("RoomLayer:onButtonClickedEvent:"..tag)
...
function RoomLayer:createTableBg()
	cclog("function RoomLayer:createTableBg() ==>")
	...
	local customRoomFile = "game/" .. modulestr .. "src/views/GameRoomLayer.lua"
	cclog("RoomLayer:createTableBg===customRoomFile: " .. customRoomFile)
	...
	if nil == sp_table or nil == chair_pos then
		cclog("RoomLayer:createTableBg default param")
		...
		ccui.Text:create("", "fonts/yuanti_sc_light.ttf", 16)
			:addTo(sp_table)
...
function RoomLayer:updateTable( spTable ,cbStatus )
	...
	cclog("function RoomLayer:updateTable( spTable ,cbStatus ) ==>")
...
function RoomLayer:getCellWidth()
	...
	cclog("function RoomLayer:getCellWidth() ==>")
...
function RoomLayer:onFindTable()
	-- body
	cclog("function RoomLayer:onFindTable() ==>")
end

--加密桌子
function RoomLayer:onCreateLockTable()
	-- body
	cclog("function RoomLayer:onCreateLockTable() ==>")
end

--加密桌子
function RoomLayer:onEnterLockTable(tableid,chairid)

	cclog("function RoomLayer:onEnterLockTable(tableid,chairid) ==>")
...
function RoomLayer:onSitDown(tableid,chairid)
	cclog("function RoomLayer:onSitDown table"..tableid.." chair"..chairid)
...
function RoomLayer:onKeyBack()
	cclog("function RoomLayer:onKeyBack() ==>")
	return not self.m_bEnableKeyBack
end

function RoomLayer:upDataTableStatus(tableid,tablestatus)
	cclog("function RoomLayer:upDataTableStatus(tableid,tablestatus) ==>")
...
function RoomLayer:onEventUserEnter(tableid,chairid,useritem)
	cclog("function RoomLayer:onEventUserEnter(tableid,chairid,useritem) ==>")
...
function RoomLayer:onEventUserStatus(useritem,newstatus,oldstatus)
	...
	cclog("function RoomLayer:onEventUserStatus(useritem,newstatus,oldstatus) ==>")
...
function RoomLayer:onGetTableInfo()
	...
	cclog("function RoomLayer:onGetTableInfo() ==>")
...
function RoomLayer:onQuickStart()
	cclog("function RoomLayer:onQuickStart() ==>")
...
function RoomLayer:onEnterTable()
	cclog("function RoomLayer:onEnterTable() ==>")
	self._scene:onEnterTable()
end

function RoomLayer:onReQueryFailure(code,msg)
	cclog("function RoomLayer:onReQueryFailure(code,msg) ==>")
	self:dismissPopWait()
