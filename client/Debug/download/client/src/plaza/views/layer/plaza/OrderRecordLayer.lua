...
function OrderRecordLayer:onEnterTransitionFinish()
	cclog("function OrderRecordLayer:onEnterTransitionFinish() ==>")
...
function OrderRecordLayer:onExitTransitionStart()
	cclog("function OrderRecordLayer:onExitTransitionStart() ==>")

    return self
end

function OrderRecordLayer:ctor(scene)
	cclog("function OrderRecordLayer:ctor(scene) ==>")
	...
	--无记录提示
	self._nullTipLabel = cc.Label:createWithTTF("没有兑换记录","fonts/yuanti_sc_light.ttf",32)
			:move(yl.WIDTH/2,326)
...
function OrderRecordLayer:onUpdateShow()
	...
	cclog("function OrderRecordLayer:onUpdateShow() ==>")
...
function OrderRecordLayer.cellSizeForTable(view, idx)
	cclog("function OrderRecordLayer.cellSizeForTable(view, idx) ==>")
  	return 1161 , 76
end

--子视图数目
function OrderRecordLayer.numberOfCellsInTableView(view)
	cclog("function OrderRecordLayer.numberOfCellsInTableView(view) ==>")
	return #view:getParent()._OrderRecordList
end
	
--获取子视图
function OrderRecordLayer.tableCellAtIndex(view, idx)
	cclog("function OrderRecordLayer.tableCellAtIndex(view, idx) ==>")
	...
	if not cell then
		...
		cc.Label:createWithTTF(date,"fonts/yuanti_sc_light.ttf",28)
			:move(118,height/2)
			:setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
			:setTextColor(cc.c4b(206,175,255,255))
			:setAnchorPoint(cc.p(0.5,0.5))
		...
		cc.Label:createWithTTF(item.name,"fonts/yuanti_sc_light.ttf",28)
		...
		cc.Label:createWithTTF(string.formatNumberThousands(item.count,true,","),"fonts/yuanti_sc_light.ttf",28)
		...
		cc.Label:createWithTTF(string.formatNumberThousands(item.price,true,",").."元宝","fonts/yuanti_sc_light.ttf",28)
		...
		cc.Label:createWithTTF(item.tradeType,"fonts/yuanti_sc_light.ttf",28)
