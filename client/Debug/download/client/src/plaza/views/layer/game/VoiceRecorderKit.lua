-- Name: VoiceRecorderLayer
-- Func: 录音界面
-- Author: Johny


-- 存储路径
AUDIO_DOWN_RECORD = device.writablePath .. "r__audio_down_record/"
AUDIO_SAVE_RECORD = device.writablePath .. "r__audio_save_record/"
...
function VoiceRecorderLayer:ctor( parent, gameFrame )
    cclog("function VoiceRecorderLayer:ctor( parent, gameFrame ) ==>")

    self.parent = parent
    -- 加载csb资源
    local rootLayer, csbNode = ExternalFun.loadRootCSB("plazanew/VoiceRecordLayer.csb", self )
    self.m_actAni = ExternalFun.loadTimeLine( "plazanew/VoiceRecordLayer.csb" )
    ...
    -- 移除文件
    cc.FileUtils:getInstance():removeFile(AUDIO_SAVE_RECORD .. "record.mp3")
    RECORD_STATE = true
    -- 权限检查
...
function VoiceRecorderLayer:onCountDown(dt)
    ...
    cclog("function VoiceRecorderLayer:onCountDown(dt) ==>")
...
function VoiceRecorderLayer:removeRecorde()
    ...
    cclog("function VoiceRecorderLayer:removeRecorde() ==>")
...
function VoiceRecorderLayer:cancelVoiceRecord()
    ...
    cclog("function VoiceRecorderLayer:cancelVoiceRecord() ==>")
...
function VoiceRecorderKit.init()
    ...
    cclog("function VoiceRecorderKit.init() ==>")
    -- 配置路径
    AudioRecorder:getInstance():init(AUDIO_SAVE_RECORD, AUDIO_DOWN_RECORD)
end

function VoiceRecorderKit.createRecorderLayer( parent, gameFrame )

    cclog("function VoiceRecorderKit.createRecorderLayer( parent, gameFrame ) ==>")
...
function VoiceRecorderKit.startPlayVoice( spath )
    ...
    cclog("function VoiceRecorderKit.startPlayVoice( spath ) ==>")
...
function VoiceRecorderKit.finishPlayVoice()
    ...
    cclog("function VoiceRecorderKit.finishPlayVoice() ==>")
