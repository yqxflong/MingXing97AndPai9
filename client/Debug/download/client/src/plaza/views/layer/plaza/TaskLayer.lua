...
function TaskLayer:onEnterTransitionFinish()
    ...
    cclog("function TaskLayer:onEnterTransitionFinish() ==>")
...
function TaskLayer:onExitTransitionStart()
    cclog("function TaskLayer:onExitTransitionStart() ==>")
...
function TaskLayer:ctor(scene, gameFrame)
    ...
    cclog("function TaskLayer:ctor(scene, gameFrame) ==>")
...
function TaskLayer:onHandleTask(wTaskID,cbTaskStatus)
    ...
    cclog("function TaskLayer:onHandleTask(wTaskID,cbTaskStatus) ==>")
...
function TaskLayer:onButtonClickedEvent(tag,sender)
    ...
    cclog("function TaskLayer:onButtonClickedEvent(tag,sender) ==>")
...
function TaskLayer:onTaskCallBack(result,message)
	...
	cclog("function ======== TaskLayer:onTaskCallBack ========")
    local bRes = false
	...
	elseif result == 2 then
        cclog(" taskid " .. self._wTaskID .. " command " .. self._wCommand)
...
function TaskLayer.cellSizeForTable(view, idx)
    ...
    cclog("function TaskLayer.cellSizeForTable(view, idx) ==>")
...
function TaskLayer:numberOfCellsInTableView(view)
    ...
    cclog("function TaskLayer:numberOfCellsInTableView(view) ==>")
...
function TaskLayer:tableCellAtIndex(view, idx)
    ...
    cclog("function TaskLayer:tableCellAtIndex(view, idx) ==>")
...
function TaskLayer:createTaskItem(cell, item, view)
    ...
    cclog("function TaskLayer:createTaskItem(cell, item, view) ==>")
    ...
    display.newSprite("Task/text_task_task.png")
        ...
        :setTag(4)
        :addTo(cell)
    local cpName = ClipText:createClipText(cc.size(210,30), item.szTaskName, "fonts/yuanti_sc_light.ttf", 25)
    ...
    display.newSprite("Task/frame_task_progress_1.png")
        ...
        :addTo(cell)
    --进度文字
    cc.Label:createWithTTF(""..item.wTaskProgress.."/"..item.wTaskObject, "fonts/yuanti_sc_light.ttf", 14)
    ...
    display.newSprite("Task/text_task_time.png")
        :move(550,80)
        :addTo(cell)
    cc.Label:createWithTTF("无时限", "fonts/yuanti_sc_light.ttf", 24)
            :setAnchorPoint(cc.p(0,0.5))
            :move(593,80)
            :setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
