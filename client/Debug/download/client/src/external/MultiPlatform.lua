...
function MultiPlatform:getExtralDocPath()
	cclog("MultiPlatform:getExtralDocPath===>")
	local plat = self:getSupportPlatform()
	local path = device.writablePath
	cclog("MultiPlatform:getExtralDocPath===plat: " .. plat .. "=path: " .. path)
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getExtralDocPath then
		path = g_var(PLATFORM[plat]).getExtralDocPath( )
	else
		cclog("undefined funtion or unknow platform ==> " .. plat)
	end	

	if nil ~= PLATFORM[plat] and false == cc.FileUtils:getInstance():isDirectoryExist(path) then
		cclog("MultiPlatform:getExtralDocPath==createDirectory==" .. PLATFORM[plat])
...
local l = 
		...
		cclog(MultiPlatform:getInstance():metersBetweenLocation(l))
