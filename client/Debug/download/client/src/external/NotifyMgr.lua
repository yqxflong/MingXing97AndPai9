...
function NotifyMgr:registerNotify(notify) 
    ...
    if nil == mNotify then
        cclog("register main ==> " .. notify.main .. " sub ==> " .. notify.sub .. " name ==> " .. notify.name)
        table.insert(self.m_tabNofityMgr, notify)
    else
        cclog("update main ==> " .. notify.main .. " sub ==> " .. notify.sub .. " name ==> " .. notify.name)
        mNotify = notify
    end
end
...
function NotifyMgr:unregisterNotify(main, sub, name)
    ...
    for k,v in pairs(self.m_tabNofityMgr) do
        if v.main == main and v.sub == sub and v.name == name then
            idx = k
            cclog("unregister main ==> " .. main .. " sub ==> " .. sub .. " name ==> " .. name)
        end
    end
    if nil ~= idx then
    ...
    for k,v in pairs(self.m_tabNofityMgr) do
        if v.main == main and v.sub == sub then
            ...
            if true == v.active then
                ...
                cclog("cache notify: main ==> " .. main .. " sub ==> " .. sub)
