...
function ExternalFun.read_datahelper( param )
	if debug_mode then
		cclog("read: " .. param.strkey .. " helper");
	end
	...
	if nil ~= param.lentable then		
			...
			for i=1,entryLen do
				...
				if debug_mode then					
					if type(entry) == "boolean" then
						cclog("value:".. (entry and "true" or "false"))
					else
						cclog("value:" .. entry);
					end
				end
		...
		if debug_mode then
			...
			if type(value) == "boolean" then
				cclog("value:".. (value and "true" or "false"))
			else
				cclog("value:" .. value);
			end		
			return value;
...
function ExternalFun.read_netdata( keyTable, dataBuffer )
	...
	for k,v in pairs(keyTable) do
		...
		elseif "short" == keyType then
			...
			cclog("read_netdata error: key==>" .. key .. "; type==>" .. keyType);
		end
		if nil ~= keyFun then
...
function ExternalFun.loadRootCSB( csbFile, parent )
	local rootlayer = ccui.Layout:create()
		:setContentSize(G_WIDTH_GLVIEW, G_HEIGHT_GLVIEW) --这个是资源设计尺寸
		:setScale(yl.WIDTH / G_WIDTH_GLVIEW);
	if nil ~= parent then
...
function ExternalFun.loadLexicon( )
	...
	cclog("load time ==> " .. endTime - startTime)
end
...
function ExternalFun.isContainBadWords( str )
	...
	local endTime = os.clock()
	cclog("excute time ==> " .. endTime - startTime)
...
function ExternalFun.isContainEmoji(str)
	...
	cclog("origin " .. origincount)
	local count = 0
	for unchar in string.gfind(str, "[%z\25-\57\64-\126\227-\240][\128-\191]*") do
		--[[cclog(string.len(unchar))
		cclog(string.byte(unchar))]]
		if string.len(unchar) < 4 then
			count = count + 1
		end		
	end
	cclog("newcount " .. count)
...
function TouchFilter:onTouchEnded(touch, event)
	cclog("TouchFilter:onTouchEnded")
